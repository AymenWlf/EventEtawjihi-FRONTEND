...
const OrientationReport: React.FC<OrientationReportProps> = ({ userData, language = 'fr', onRestart }) => {
      ...
      try {
        ...
        if (response.data.success && response.data.hasTest) {
          ...
          console.log("Structure des données API:", {
            ...
            aptitude: testData.currentStep.aptitude,
          ...
          if (userReportData && Object.keys(userReportData).length > 0) {
            console.warn("⚠️ L'API n'a pas retourné de données - Utilisation des données partielles fournies en props comme fallback:", userReportData);
            setUserReportData(userReportData);
          } else {
        ...
        if (userReportData && Object.keys(userReportData).length > 0) {
          console.warn("⚠️ Erreur API - Utilisation des données partielles des props comme fallback:", userReportData);
          setUserReportData(userReportData);
        } else {
  ...
  const printReport = () => {
    ...
    const printableContent = ReactDOMServer.renderToString(
      <PrintableReport userData={userReportData} language={language} />
    );
  ...
  const getDynamicRecommendations = (userReportData: any, t: any) => {
    ...
    // Aptitudes
    if ((userReportData.aptitudeScores?.overallScore || 0) < 30) {
      recs.push("Renforcez vos compétences logiques et verbales par des exercices ciblés.");
    } else if ((userReportData.aptitudeScores?.overallScore || 0) > 70) {
    ...
    <div className={`min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 print:bg-white ${language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      <div className="max-w-6xl mx-auto px-2 sm:px-4 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6 md:space-y-8 print:px-0 print:py-4">
        ...
        <section className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 md:mb-8">
          ...
          {/* Bouton pour afficher/masquer les questions/réponses */}
          <button
            onClick={() => toggleQASection('aptitude')}
            className={`flex items-center gap-2 w-full justify-between px-3 py-2 bg-orange-50 rounded-lg text-sm font-medium text-orange-700 mb-3 hover:bg-orange-100 transition-colors ${language === 'ar' ? 'flex-row-reverse' : ''}`}
          >
            <span>{t.questionsAnswers}</span>
            {expandedQASections.aptitude
              ? <ChevronUpIcon className="w-5 h-5" />
              : <ChevronDownIcon className="w-5 h-5" />
            }
          </button>

          {/* Contenu des questions/réponses - visible uniquement si expandedQASections.aptitude est true */}
          {expandedQASections.aptitude && (
            <div className="space-y-2 mb-4">
              {Object.entries(userReportData.aptitudeScores?.detailedResponses || {}).map(([type, questions]: [string, any]) => (
          ...
          {/* Section visualisation Aptitudes - toujours visible */}
          <h3 className={`font-semibold mb-3 sm:mb-4 text-sm sm:text-base ${language === 'ar' ? 'text-right' : ''}`}>
            {language === 'ar' ? "تصور ملف القدرات" : "Visualisation du profil d'aptitudes"}
          </h3>
          <BarChart
            data={userReportData.aptitudeScores?.scores || {}}
