...
// Définition des étapes du test d'orientation
const testSteps = [
  {
    id: 'personalInfo',
    icon: UserIcon,
...
const translations = {
  fr: {
    ...
    estimatedTime: "Durée estimée : 30-50 minutes",
  ...
  ar: {
    ...
    estimatedTime: "المدة المقدرة: 30-50 دقيقة",
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
  ...
  const getStepStatus = (stepId: string, testData: any, completedSteps: string[]) => {
    ...
    switch (stepId) {
      case 'personalInfo':
        if (testData?.currentStep?.personalInfo) {
          // Vérifier si l'étape est explicitement marquée comme complétée
          if (testData.currentStep.personalInfo.isCompleted) {
            return 'completed';
          }

          // Vérifier si les données sont remplies (présence de personalInfo.personalInfo)
          if (testData.currentStep.personalInfo.personalInfo &&
            Object.keys(testData.currentStep.personalInfo.personalInfo).length > 0) {

            // Vérifier si les champs essentiels sont remplis
            const info = testData.currentStep.personalInfo.personalInfo;
            const essentialFields = ['firstName', 'lastName', 'age', 'studyLevel', 'bacType'];
            const allEssentialFieldsFilled = essentialFields.every(field => !!info[field]);
        ...
        break;
      case 'riasec':
        if (testData?.currentStep?.riasec) {
          return 'current';
        }
        break;
      case 'personality':
        if (testData?.currentStep?.personality) {
          return 'current';
        }
        break;
      case 'aptitude':
        ...
        break;
      case 'interests':
        if (testData?.currentStep?.interests?.interests?.fieldInterests &&
          Object.keys(testData.currentStep.interests.interests.fieldInterests).length > 0) {
          return 'current';
        }
        break;
      case 'careerCompatibility':
        if (testData?.currentStep?.careerCompatibility?.careers?.careerAttractions &&
          Object.keys(testData.currentStep.careerCompatibility.careers.careerAttractions).length > 0) {
          return 'current';
        }
        break;
      case 'constraints':
        if (testData?.currentStep?.constraints?.constraints) {
        ...
        break;
      case 'languageSkills':
        if (testData?.currentStep?.languageSkills?.languages) {
          const languageData = testData.currentStep.languageSkills.languages;
          if ((languageData.selectedLanguages && languageData.selectedLanguages.length > 0) ||
      ...
      try {
        ...
        if (response.data.success && response.data.hasTest) {
          ...
          testSteps.forEach(step => {
            calculatedStepStatuses[step.id] = getStepStatus(step.id, testData, completedSteps);
          });
  ...
  const handleStartSpecificStep = async (stepId: string) => {
    ...
    try {
      // Récupérer d'abord l'état actuel du test
      ...
      if (response.data.success) {
        ...
        console.log("Données envoyées à onComplete:", testData);
  ...
  const getStepDescriptionFr = (stepId: string) => {
    switch (stepId) {
      ...
      case 'aptitude':
        return "Évaluez vos compétences cognitives dans différents domaines académiques et professionnels.";
      case 'interests':
  ...
  const getStepDescriptionAr = (stepId: string) => {
    switch (stepId) {
      ...
      case 'aptitude':
        return "قيّم مهاراتك المعرفية في مختلف المجالات الأكاديمية والمهنية.";
      case 'interests':
  ...
  const getStepDuration = (stepId: string) => {
    switch (stepId) {
      ...
      case 'aptitude':
        return currentLanguage === 'ar' ? "10-15 دقائق" : "10-15 minutes";
      case 'interests':
        return currentLanguage === 'ar' ? "4-6 دقائق" : "4-6 minutes";
      case 'careerCompatibility':
        return currentLanguage === 'ar' ? "5-10دقائق" : "5-10minutes";
      case 'constraints':
                        ...
                        {(stepStatus !== 'locked' || (step.id === 'personalInfo' && !testStatus)) && (
                          <button
                            onClick={() => {
                              ...
                              } else if (step.id === 'personalInfo' && selectedLanguage) {
                                handleStartTest();
                              } else if (stepStatus !== 'locked') {
                                handleStartSpecificStep(step.id);
                              }
                            }}
                            className={`w-full sm:w-auto text-xs font-medium px-3 py-1.5 sm:py-1 rounded-lg transition-all hover:scale-105 ${!selectedLanguage && step.id === 'personalInfo' ? 'bg-amber-600 text-white hover:bg-amber-700' : stepStatus === 'completed' ? 'bg-green-600 text-white hover:bg-green-700' : stepStatus === 'current' ? 'bg-amber-600 text-white hover:bg-amber-700 animate-pulse shadow-md' : `bg-blue-600 text-white hover:bg-blue-700`} ${isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:shadow-lg'}`}
                          >
                            {isLoading ? (
